...
import { CheckboxFillIcon, CheckboxGrayIcon, CloseIcon, StepOneInstructor } from "@/assets/icons";
...
const createCheckedTerms = (value: boolean) =>
  Object.fromEntries(INSTRUCTOR_TERMS.map(({ id }) => [id, value])) as Record<
    InstructorTermsId,
    boolean
  >;
...
const Page = () => {
  ...
  const [selectedTermId, setSelectedTermId] = useState<InstructorTermsId | null>(null);
  const [checkedTerms, setCheckedTerms] = useState<Record<InstructorTermsId, boolean>>(() =>
    createCheckedTerms(false),
  );

  const selectedTerm = INSTRUCTOR_TERMS.find(({ id }) => id === selectedTermId);
  const isAllAgreed = INSTRUCTOR_TERMS.every(({ id }) => checkedTerms[id]);
  ...
  const toggleTerm = (termId: InstructorTermsId) => {
          ...
          <section className="flex w-118 flex-col gap-12">
            ...
            <div className="flex w-full items-start justify-between">
              ...
              <Button
                ...
                type="button"
                onClick={() => {
                  if (isNextEnabled) router.push("/signup/instructor/step2");
                }}
              >
                다음
              </Button>
