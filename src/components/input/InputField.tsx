...
const InputField = ({
  ...
  const messageId = id != null && visibleMessage ? `${id}-message` : undefined;
  ...
  return (
    <div className={cn("flex w-full flex-col gap-2", wrapperClassName)}>
      {label != null && (
        <label className="text-body2-r text-gray-70" htmlFor={id}>
          {label}
        </label>
      )}
      <div
      ...
      {visibleMessage && (
        <p
          className={cn("text-heading3-m", hasError ? "text-red-main" : "text-gray-60")}
          id={messageId}
        >
          {visibleMessage}
        </p>
